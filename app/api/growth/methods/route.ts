...
export async function PATCH(req: Request) {
  ...
  if (methodApplicability(account.persona, parsed.data.methodId) !== "explore") return NextResponse.json({ error: "not_explore_method" }, { status: 409 });
  const suggestion = account.weekly_review?.promotion_suggestions.find((item) => item.method_id === parsed.data.methodId);
  if (!suggestion) return NextResponse.json({ error: "promotion_threshold_not_met" }, { status: 409 });
  const next = { ...account, method_overrides: { ...(account.method_overrides ?? {}), [parsed.data.methodId]: "default" as const }, updated_at: new Date().toISOString() };
