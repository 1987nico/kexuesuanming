...
/**
 * 操作员发起「完整咨询报告测评」：创建（或复用）滑卡会话并预生成第一批方向。
 */
export async function POST(req: Request) {
  ...
  const surveyPath = `/survey/directions/${profile.id}`;
  ...
  if (session.status === "active" && session.rounds_generated === 0) {
    try {
      ...
      return NextResponse.json(
        { error: "direction_generation_failed", message: "首批大报告测评选项生成失败，请稍后重试。" },
        { status: 500 }
      );
    }
    ...
    if (session.rounds_generated === 0) {
      return NextResponse.json(
        { error: "direction_generation_failed", message: "首批大报告测评选项还没有生成完成，请稍后重试。" },
        { status: 500 }
      );
    }
  }
