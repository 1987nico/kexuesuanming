...
const UNNATURAL_JARGON_PATTERNS: RegExp[] = [
  /邪修/gu,
  /(?:一|二|三|四|五|六|七|八|九|十|\d+)步[^，。！？?；;]{0,4}漏斗/gu,
  /饭局判胜算/gu,
  ...
  /写推(?!荐)/gu,
  // 生成模型偶尔把方法标签“送资料”直接压进标题，形成“送项目证据账本”
  // 这类运营术语泄漏；自然标题可以写“送你/给你”，但不能裸用“送+名词”。
  /^送(?!你|给)/gu,
  // 下列都是线上真实生成里出现过的机械压缩。逐个收进底线规则，避免模型
  // 终审偶发把“字数够短”误当成“中文自然”。
  /真假意/gu,
  /高胜算(?:转型|求职|跳槽)机会/gu,
  /最易失效认证日期/gu,
...
export function nativeTitleSemanticTopicKeys(methodId: string, value: string) {
  ...
  add(
    "conflict:progress_signal_but_role_mismatch",
    /(?:笔试|面试|工签|内推|录用|offer|流程|进展|机会)/iu.test(title)
      && /(?:岗位|职位|方向|岗)/u.test(title)
  ...
  add(
    "decision:familiar_team_vs_strong_manager",
    /(?:熟人团队|熟悉的团队|老团队|原团队)/u.test(title)
      && /(?:强上级|更强的上司|强老板|好上司|强领导|更强的领导)/u.test(title),
  );
  add(
